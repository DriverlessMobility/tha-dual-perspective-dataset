...
nsweeps = 1
data_root = "/data/THA_DualPerspective_Dataset/ITS-S"
...
pointcloudrange = [-74.88, -74.88, -2, 74.88, 74.88, 4.0]
...
model = dict(
    type="PointPillars",
    pretrained="/data/workspace/work_dir/waymoTraining/latest.pth",
    reader=dict(
        ...
        pc_range=tuple(pointcloudrange)
    ),
...
test_cfg = dict(
    post_center_limit_range=pointcloudrange,
    nms=dict(
    ...
    score_threshold=0.1,
    pc_range=[pointcloudrange[0], pointcloudrange[1]],
    out_size_factor=get_downsample_factor(model),
...
lr_config = dict(
    type="one_cycle", lr_max=0.00003, moms=[0.95, 0.85], div_factor=10.0, pct_start=0.4,
)
